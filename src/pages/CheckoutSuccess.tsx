...
export default function CheckoutSuccess() {
    ...
    useEffect(() => {
        const finalizePurchase = async () => {
            ...
            try {
                ...
                // 1. Check if already recorded to avoid duplicates
                ...
                // 2. Insert into user_simulados
                // In a real production app, you'd verify the session server-side via webhook.
                // For this demo/dev implementation, we'll insert directly.
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-white text-center">
                <Loader2 className="size-12 text-[#f3ec05] animate-spin mb-6" />
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-white text-center">
                ...
                <button
                    onClick={() => navigate('/')}
                    className="bg-white/5 text-white px-8 py-4 rounded-2xl font-black uppercase italic tracking-widest border border-white/10"
                >
                    Voltar para Início
                </button>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-white text-center">
            <div className="size-24 bg-emerald-500/20 text-emerald-400 rounded-[2.5rem] flex items-center justify-center mb-8 shadow-2xl shadow-emerald-500/10">
            ...
            <div className="flex flex-col gap-4 w-full max-w-xs">
                ...
                <button
                    onClick={() => navigate('/')}
                    className="text-slate-500 font-black uppercase tracking-widest text-xs hover:text-white transition-colors py-4"
                >
